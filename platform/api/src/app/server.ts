...
import { validateTemplateCreate } from "../modules/config/template-validator";
...
const route = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
  ...
  if (method === "GET" && url.pathname === "/api/pool") {
    const themesParam = url.searchParams.get("themeIds") ?? "";
    const themeIds = themesParam.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
    send(res, 200, poolStats(themeIds.length > 0 ? themeIds : undefined));
    return;
  }
  ...
  // Ephemeral template preview: generate a one-question probe from a template
  // body sent inline — nothing is written to the templates table, no version bump.
  if (method === "POST" && url.pathname === "/api/generation/preview") {
    try {
      const raw = (await readJsonBody(req)) as Record<string, unknown>;
      const input = validateTemplateCreate(raw["template"]);
      const config: GenerationConfig = {
        title: `Тест шаблону: ${input.title.slice(0, 60)}`,
        variantCount: 1,
        questionsPerVariant: 1,
      ...
      const job = jobStore.create(config);
      void executeGenerationJob(job.jobId, config, [preview]);
      send(res, 202, { jobId: job.jobId, status: job.status });
    } catch (e) {
      send(res, 400, { code: "INVALID_TEMPLATE", message: e instanceof Error ? e.message : "Invalid request" });
    }
    return;
  }
  ...
  // Unmatched API paths must return JSON 404, never fall through to the SPA
