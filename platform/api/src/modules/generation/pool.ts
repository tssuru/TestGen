...
const beforeText = (beforeRaw: string | null, fallback: string): { before: string; promptText: string } => {
  ...
  const cleaned = stripEngineMarkers(beforeRaw)
    ...
    .replace(/\s+/g, " ")
    .trim();
  const promptText = cleaned.length > 0 ? cleaned : fallback;
...
export const poolStats = (): { closedCount: number; openCount: number } => {
  ...
  try {
    ...
    const byKind = getRepositories().templates.countByKind();
    if (byKind.closed + byKind.open > 0) {
