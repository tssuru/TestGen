...
const isCommentLine = (l: string): boolean =>
  /^\s*\/\//.test(l) ||
  // LaTeX-style % comments, but never engine markers (%begin/%end/%beginitem/%enditem/%goodpath)
  (/^\s*%/.test(l) && !/^\s*%(?:begin|end|goodpath)/.test(l));

const readOpenCode = (abs: string): string => {
  try {
    // Normalize CRLF and drop full-line comments (engine docs are Russian-language;
    // they must not surface in the template editor). Inline // is kept — it is
    // floor division in the Python snippets.
