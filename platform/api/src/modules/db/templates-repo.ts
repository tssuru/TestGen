  ...
  return {
    ...
    list(filter: TemplateListFilter = {}): TemplateSummary[] {
      ...
      if (filter.search !== undefined && filter.search.length > 0) {
        clauses.push("t.title LIKE ? ESCAPE '\\'");
    ...
    update(id: string, patch: TemplateUpdateInput): Template | null {
      ...
      const now = nowIso();
      // An edited imported template no longer matches its original content —
      // clear content_key so a future re-import re-creates the pristine copy.
      if (bodyChanged) {
        db.prepare(
          `UPDATE templates SET title = ?, body = ?, version = ?, updated_at = ?, content_key = NULL WHERE id = ?`,
    ...
    countByKind(themeIds?: readonly string[]): { closed: number; open: number } {
      const withThemes = themeIds !== undefined && themeIds.length > 0;
      const r = withThemes
        ? db
            .prepare<unknown[], { kind: TemplateKind; c: number }>(
              `SELECT t.kind AS kind, COUNT(DISTINCT t.id) AS c FROM templates t
               JOIN template_themes tt ON tt.template_id = t.id
               WHERE tt.theme_id IN (${themeIds.map(() => "?").join(",")})
               GROUP BY t.kind`,
            )
            .all(...themeIds)
        : db
