...
import type { ApiError, ArtifactFormat, ArtifactInfo, BridgeGenerateRequest, GenerationConfig, Template } from "@quiz/contracts";
...
export const executeGenerationJob = async (
  ...
  // Ephemeral templates (e.g. editor preview) — bypass DB selection entirely
  inlineTemplates?: readonly Template[],
): Promise<void> => {
  try {
    ...
    const templates = inlineTemplates !== undefined && inlineTemplates.length > 0
      ? inlineTemplates
      : selectTemplates(config, extraFilter);
