...
// Ephemeral one-question probe from an unsaved template body — no version bump
export const createPreviewJob = (template: TemplateCreateInput): Promise<JobCreateResponse> =>
  request(`${BASE}/generation/preview`, jsonBody("POST", { template }));
...
export const getPoolStats = (themeIds: readonly string[] = []): Promise<PoolStats> =>
  request(themeIds.length > 0
    ? `${BASE}/pool?themeIds=${themeIds.map(encodeURIComponent).join(",")}`
    : `${BASE}/pool`);
