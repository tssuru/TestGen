...
export const GeneratorPage = () => {
  ...
  // Pool counters follow the selected themes
  const themeKey = (config.themeIds ?? []).join(",");
  useEffect(() => {
    void getPoolStats(config.themeIds ?? []).then(setPool).catch(() => setPool(null));
  ...
  const generate = async (): Promise<void> => {
    ...
    try {
      const poolProblem = await checkPoolSize(config);
