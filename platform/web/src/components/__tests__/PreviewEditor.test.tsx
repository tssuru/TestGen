...
vi.mock("../../api", () => ({
  ...
  regenerateItem: vi.fn(async () => ({ jobId: "child-2", status: "queued", parentJobId: "job-1", replacedIndex: 1 })),
...
describe("PreviewEditor", () => {
  ...
  it("random swap (dice) calls regenerateItem with the row index and navigates to the child job", async () => {
    const api = await import("../../api");
    renderEditor();
    const user = userEvent.setup();
    await screen.findByText("Обрати правильні твердження");
    await user.click(screen.getByTestId("pe-random-2"));

    await waitFor(() => { expect(api.regenerateItem).toHaveBeenCalledWith("job-1", 2); });
    await waitFor(() => { expect(mockNavigate).toHaveBeenCalledWith("/jobs/child-2"); });
  });

  it("random swap shows engine errors when the regenerated job fails", async () => {
    const api = await import("../../api");
    (api.getJob as ReturnType<typeof vi.fn>).mockResolvedValue({
      jobId: "child-2", status: "failed", progress: 100,
      config: {}, errors: [{ code: "ENGINE_ERROR", message: "движок впав" }],
      diagnostics: [], artifacts: [], createdAt: "", updatedAt: "",
    });
    renderEditor();
    const user = userEvent.setup();
    await screen.findByText("Обрати правильні твердження");
    await user.click(screen.getByTestId("pe-random-1"));
    expect(await screen.findByText(/движок впав/i)).toBeInTheDocument();
