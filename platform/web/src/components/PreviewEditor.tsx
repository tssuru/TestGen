...
import { Check, Loader2, RefreshCcw, Replace, Sparkles, X } from "lucide-react";
import type { GenerationConfig, GenerationJob, TemplateSummary } from "@quiz/contracts";
import { createJob, getJob, getJobTemplates, listTemplates, type JobTemplateItem } from "../api";
...
export const PreviewEditor = ({ job }: PreviewEditorProps) => {
  ...
  const applyChanges = async (): Promise<void> => {
    ...
    try {
      ...
      let done = false;
      while (!done) {
        const cur = await getJob(created.jobId);
        setChildJob(cur);
  ...
  return (
    <div className={cardCls}>
      <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Перегляньте підібрані питання. Замініть будь-яке на інший шаблон того самого типу та натисніть «Створити фінальний бандл».
          </p>
        </div>
        {hasChanges && (
      ...
      {childJob !== null && childJob.status === "completed" && (
        <Alert kind="success"><span className="inline-flex items-center gap-1"><Check className="h-4 w-4" /> Бандл готовий, переходимо…</span></Alert>
      )}
    </div>
  );
};
